...
unsigned next_Using_Nna(const std::vector<std::vector<unsigned>>& graph, unsigned start, const std::vector<bool>& visited);
 ...
 */
std::vector<std::vector<unsigned>> update_Uniongraph(std::vector<std::vector<unsigned>>& unionGraph, const std::vector< std::vector<unsigned> > & graph, const std::vector<unsigned> & j);
